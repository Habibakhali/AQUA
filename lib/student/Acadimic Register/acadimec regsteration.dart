...
class _AcadimecRegsterationState extends State<AcadimecRegsteration> {
                              ...
                              : Expanded(
                                  ...
                                  itemBuilder: (_, index) {
                                    return Row(
                                      children: [
                                        RegisterFiles(
                                            lastfile[index].split('#')[0],
                                            (c) async {
                                          final pref = await SharedPreferences
                                              .getInstance();
                                          print('data===============>{}');
                                          print(lastfile[index]);
                                          var ddddd = await ApiManager
                                              .delAcadimicRegistery(int.parse(
                                                  lastfile[index]
                                                      .split('#')[1]));
                                          if (ddddd == 200) {
                                            print(
                                                'dele    ${ApiManager.imagesAc[index].split('#')[1]}');
                                            setState(() {
                      ...
                      Row(
                          ...
                          children: [
                            InkWell(
                              ...
                              child:
                                  Text('Select semester: ${year + selected}'),
                            ),
                          ]),
                      OutlinedButton(
                        onPressed: () {
  ...
  void validate() async {
    ...
    if (imageFile != null)
      data =
          await ApiManager.storeAcademicRegistry(imageFile!, year + selected);
    else
      ScaffoldMessenger.of(context)
