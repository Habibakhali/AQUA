...
class ForgetMyPasswordGrd extends StatefulWidget {
  static String routeName='ForgetPasswordgred';

  @override
  State<ForgetMyPasswordGrd> createState() => _ForgetMyPasswordGrdState();
}

class _ForgetMyPasswordGrdState extends State<ForgetMyPasswordGrd> {
  ...
  Future<dynamic> validation()async{
    ...
    if(foemkey.currentState!.validate()){
      ForgetPassStudent data=await ApiManager.RequestCodeToForgetPassGrd(email.text);
      if(data.errors!=null){
      ...
      Navigator.pushReplacementNamed(context, OtpFormGraduated.routeName,arguments: email.text);
        return showDialog(context: context, builder: (context) =>
            AlertDialog(
